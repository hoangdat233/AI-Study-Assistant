...
class GeminiEmbeddingProvider(BaseEmbeddingProvider):
    """Google Gemini text embedding provider using direct REST API calls."""
        ...
        try:
            ...
            if status_code in (401, 403):
                ...
                raise HTTPException(
                    ...
                    detail="Embedding rate limit exceeded. Please try again later.",
                )
            else:
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
            ...
            raise HTTPException(
                ...
                detail="Embedding provider network request timed out.",
            )

    def embed_texts(self, texts: list[str]) -> list[list[float]]:
        if not texts:
            return []
        # Batch embed or map embed_text safely
...
class MockEmbeddingProvider(BaseEmbeddingProvider):
    ...
    def embed_text(self, text: str) -> list[float]:
        # Generate pseudo-deterministic 3072 float vector based on text hash
        val = (abs(hash(text)) % 1000) / 1000.0
        vec = [val] * EMBEDDING_DIMENSION
        # Add small index variation for vector operations
